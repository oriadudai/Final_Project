...
> (`arch_reheartnet`) was kept training to completion — **confirmed finished, all 8 folds,
> 2026-06-21** — and is instead analysed as part of the per-subject calibration study
> (`diag_calib_loss_ablation.py`, Final Report Section 4.7) rather than anchoring a 4-way
...
- **`diag_calib_loss_ablation.py` — `arch_reheartnet` confirmed at full scale (2026-06-21).**
  Once `arch_reheartnet`'s 8-fold training finished (see Step 4 status above), the 3-way
  calibration-loss ablation (MSE / Huber / Composite) was re-run on it at full $N=53$
  (previously only $n=35$, folds 0-4, was available) — replicating the partial-tradeoff
  story of Final Report Section 4.7 at full cohort scale: composite calibration is the only
  objective that doesn't push EMD/KS *above* baseline (EMD +4%, KS −2%; MSE/Huber push both
  +36-109% above baseline), at the cost of a smaller PRD gain (94.5% vs. ~82%).
  One finding changed materially vs. the $n=35$ pilot: the Spearman rank-correlation cost of
  composite calibration on MI, previously non-significant ($\rho=0.269$), is now significant
  ($\rho=0.375^{**}$) at full cohort scale — composite calibration of `arch_reheartnet` is
  significant on *every* PTB-XL class at $N=53$, just weaker on MI than the other two models.
- **`print_diag_class_table.py`** / **`print_diag_spearman_table.py`** — print the per-class
  mean-absolute-error and Spearman-rank-correlation tables (ASCII + LaTeX) from one or more
  `calib_loss_ablation_summary.json` files. The Spearman script is new (no prior version
  existed in this repo); the class-error script mirrors its CLI.
- **`plot_diag_class_heatmap.py`** — renders the per-class error table above as a
  model × calibration heatmap (`results/figures/diag_class_heatmap.pdf`).
- **`plot_distortion_perception_plane.py`** — the headline Blau–Michaeli-style scatter:
  PRD (distortion) vs. EMD (perception), one point per model × calibration condition, with
  arrows from baseline to each calibrated variant (`results/figures/distortion_perception_plane.png`,
  Final Report Figure 4.7.x). Locked design: PRD-vs-EMD only — `diag_kl`/`flip_rate` are
  deliberately left in the per-class tables rather than folded into this plot (see caption).
- **`diag_plot_calibration_multi_model.py`** — the transpose of
  `diag_plot_calibration_multi_loss.py`: fixes *one subject* and varies the *three model
  variants* (one panel per model, each showing real ECG vs. baseline vs. all three
  calibration objectives). Useful for inspecting how a single subject's reconstruction
  differs across models — note that, being a single-window waveform plot, it can only show
  the *distortion* axis and will always make composite calibration look "worst" pointwise
  for `arch_reheartnet`; it cannot show the EMD/KS advantage composite buys in exchange
  (that requires the distribution-level metrics above, not a single window).
